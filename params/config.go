...
type ChainConfig struct {
	...
	Ren    *RenConfig    `json:"ren,omitempty"`
}
...
// RenConfig is the consensus engine configs for ren darknodes protected sealing.
type RenConfig struct {
	Period uint64 `json:"period"` // Number of seconds between blocks to enforce
	Epoch  uint64 `json:"epoch"`  // Epoch length to reset votes and checkpoint

	DNR         common.Address `json:"dnr"`                   // Ren Darknode Registry address on Ethereum
	BlockNumber uint64         `json:"dnrGenesisBlockNumber"` // Deployment block number of the DNR contract
	API         string         `json:"api"`                   // Ethereum RPC URL
}

// String implements the stringer interface, returning the consensus engine details.
func (c *RenConfig) String() string {
	return "ren"
}
